/**
 * run-migration-secfix4 — Security fixes.
 * 1) project_items_secure is recreated with security_invoker = true so the
 *    querying user's RLS is enforced (no SECURITY DEFINER view bypass).
 *    Financial columns stay revoked at column level and are exposed only
 *    through a guarded helper function.
 * 2) organization_domains.verification_token is no longer readable by
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json({ ok: true, message: "secfix4 applied" });
  } catch (err) {
