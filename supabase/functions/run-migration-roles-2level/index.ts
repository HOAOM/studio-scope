/**
 * run-migration-roles-2level — Two-level role model.
 *
 * LEVEL 1 (permissions): public.user_roles gains organization_id so a user can
 *   hold different role sets in different organizations. Multiple roles per
 *   user per org are allowed (one seat, many roles).
 * LEVEL 2 (operational): public.project_assignments maps people to a project
 *   with a specific function. Backfilled from project_members (no duplication:
 *   project_members stays the access/membership table and is kept in sync).
 *
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json({ ok: true, message: "roles-2level applied" });
  } catch (err) {
