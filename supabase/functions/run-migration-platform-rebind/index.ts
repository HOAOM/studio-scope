/**
 * run-migration-platform-rebind — re-anchors every GLOBAL (cross-organization)
 * privilege from the client role app_role='admin' to the platform layer
 * (public.platform_admins → is_platform_admin() / is_platform_owner()).
 *
 * Rules applied:
 *  - admin_* RPCs: reads   → is_platform_admin()  (staff + owner)
 *                  writes  → is_platform_owner()  (owner only)
 *  - RLS: any `has_role(auth.uid(),'admin')` branch that granted cross-org
 *    visibility is replaced by is_platform_admin(), or by the new org-scoped
 *    helper is_org_admin(org) when the capability must stay with the client
 *    admin for their OWN organization.
 *  - Tier/limit bypass in enforce_project_archive_rules() now requires
 *    is_platform_admin() instead of the client admin role.
 *
 * NOT touched (global shared master data, deliberately left as-is):
 *    master_floors / master_rooms / master_item_types / master_subcategories /
 *    cost_categories / company_settings.
 *
 * Idempotent. Platform-owner only.
 */
...
Deno.serve(async (req) => {
  ...
  const { data: isOwner, error: roleErr } = await supabase.rpc("is_platform_owner", {
    _user_id: userData.user.id,
  });
  if (roleErr || !isOwner) return json({ error: "forbidden: platform owner only" }, 403);
