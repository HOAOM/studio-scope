...
export function ApprovalGatesPanel({ items, projectId, canApprove = true, onItemClick }: ApprovalGatesPanelProps) {
  ...
  const gates = useMemo<GateGroup[]>(() => {
    ...
    return [
      ...
      { key: 'revision', label: 'In Revision', icon: <ShieldAlert className="w-4 h-4" />, color: 'text-status-at-risk', bgColor: 'bg-status-at-risk-bg', borderColor: 'border-status-at-risk/20', items: revision, description: 'Items sent back for corrections' },
      { key: 'rejected', label: 'Rejected', icon: <XCircle className="w-4 h-4" />, color: 'text-status-unsafe', bgColor: 'bg-status-unsafe-bg', borderColor: 'border-status-unsafe/20', items: rejected, description: 'Items that need complete rework' },
      { key: 'ready_order', label: 'Ready to Order', icon: <ShieldCheck className="w-4 h-4" />, color: 'text-primary', bgColor: 'bg-primary/10', borderColor: 'border-primary/20', items: readyToOrder, description: 'Approved — procurement can proceed' },
  ...
  const handleRejectClick = (e: React.MouseEvent, item: ProjectItem) => {
    e.stopPropagation();
  ...
  const handleRejectConfirm = async (reason: string) => {
    ...
    try {
      // Reject + increment revision
      const newRevision = (rejectingItem.revision_number || 1) + 1;
      ...
      // Log rejection reason in audit
      await (supabase as any).from('audit_log').insert({
        entity_type: 'item',
        entity_id: rejectingItem.id,
        action: 'reject',
        summary: `REJECTED (R${newRevision}): ${reason} | ${rejectingItem.item_code || rejectingItem.description}`,
      });

      // Create revision snapshot
      await (supabase as any).from('item_revisions').insert({
        item_id: rejectingItem.id,
        revision_number: newRevision,
        status: 'active',
        reason,
        snapshot: rejectingItem,
      });

      toast.success(`${rejectingItem.item_code || rejectingItem.description} rejected → R${newRevision}`);
  ...
  return (
    <>
                ...
                {gate.items.slice(0, 8).map(item => (
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      {item.item_code && <span className="font-mono text-[10px] text-primary font-medium">{item.item_code}</span>}
                      ...
                      {item.revision_number && item.revision_number > 1 && (
                        <Badge variant="outline" className="text-[9px] h-4 px-1 font-mono">R{item.revision_number}</Badge>
                      )}
                    ...
                    <div className="flex items-center gap-1 ml-2 shrink-0" onClick={(e) => e.stopPropagation()}>
                      {(gate.key === 'pending' || gate.key === 'revision') && canApprove && (
                        <>
                          <Button size="sm" variant="ghost" className="h-6 px-2 text-[10px] text-status-safe hover:bg-status-safe-bg"
                            onClick={(e) => { e.stopPropagation(); handleQuickApprove(item); }} disabled={updateItem.isPending}>
                            <CheckCircle2 className="w-3 h-3 mr-1" /> Approve
                          </Button>
                          <Button size="sm" variant="ghost" className="h-6 px-2 text-[10px] text-status-unsafe hover:bg-status-unsafe-bg"
                            onClick={(e) => handleRejectClick(e, item)} disabled={updateItem.isPending}>
                            <XCircle className="w-3 h-3 mr-1" /> Reject
                          </Button>
                        </>
                      )}
                      {gate.key === 'rejected' && canApprove && (
                        <Button size="sm" variant="ghost" className="h-6 px-2 text-[10px] text-status-at-risk hover:bg-status-at-risk-bg"
                          onClick={(e) => { e.stopPropagation(); handleQuickApprove(item); }} disabled={updateItem.isPending}>
                      ...
                      {gate.key === 'ready_order' && canApprove && (
                        <Button size="sm" variant="ghost" className="h-6 px-2 text-[10px] text-primary hover:bg-primary/10"
                          onClick={(e) => { e.stopPropagation(); handleAdvanceGate(item, { purchased: true } as any); }} disabled={updateItem.isPending}>
                          <ArrowRight className="w-3 h-3 mr-1" /> Mark Ordered
                        </Button>
                      )}
                      {gate.key === 'awaiting_delivery' && canApprove && (
                        <Button size="sm" variant="ghost" className="h-6 px-2 text-[10px] text-primary hover:bg-primary/10"
                          onClick={(e) => { e.stopPropagation(); handleAdvanceGate(item, { received: true, received_date: new Date().toISOString().split('T')[0] } as any); }} disabled={updateItem.isPending}>
                          <ArrowRight className="w-3 h-3 mr-1" /> Mark Received
                        </Button>
                      )}
                      {gate.key === 'ready_install' && canApprove && (
                        <Button size="sm" variant="ghost" className="h-6 px-2 text-[10px] text-status-safe hover:bg-status-safe-bg"
                          onClick={(e) => { e.stopPropagation(); handleAdvanceGate(item, { installed: true, installed_date: new Date().toISOString().split('T')[0] } as any); }} disabled={updateItem.isPending}>
      ...
      <RejectDialog
        ...
        itemLabel={rejectingItem?.item_code || rejectingItem?.description || 'Item'}
