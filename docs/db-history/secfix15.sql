-- Archived from edge function run-migration-secfix15 (già applicata in produzione)
...
AS $fn$
  SELECT
    (SELECT count(DISTINCT m.user_id) FROM public.organization_members m
      WHERE m.organization_id = p_org AND m.is_complimentary = false)::int
...
-- Solo il platform admin puo' assegnare/rimuovere il flag omaggio
...
DROP TRIGGER IF EXISTS trg_guard_complimentary_members ON public.organization_members;
CREATE TRIGGER trg_guard_complimentary_members
  BEFORE INSERT OR UPDATE ON public.organization_members
  FOR EACH ROW EXECUTE FUNCTION public.guard_complimentary_flag();

DROP TRIGGER IF EXISTS trg_guard_complimentary_invites ON public.organization_invites;
CREATE TRIGGER trg_guard_complimentary_invites
  BEFORE INSERT OR UPDATE ON public.organization_invites
  FOR EACH ROW EXECUTE FUNCTION public.guard_complimentary_flag();

-- L'accettazione invito propaga il flag omaggio
